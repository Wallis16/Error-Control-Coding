...
N = 2*10**4 #número de bits--no caso serão criados 1000000 de bits que serão reagrupados em palavras código de 7 bits
...
for yy in range(0,len(Eb_N0_dB)):
    ...
    ipM = np.reshape(ip, (int(N/4),4))
    ipC = np.dot(ipM,g)%2
    cip = np.reshape(ipC,(1,int((N/4)*7)))

    #Modulation
    s = (2*cip - 1)[0] #BPSK modulation 0 -> -1; 1 -> 0
    #Channel - AWGN
    mu, sigma = 0, 1
    n = (1/np.sqrt(2))*(np.random.normal(mu, sigma, (1,cip.shape[1])) + 1j*np.random.normal(mu, sigma, (1,cip.shape[1]))) #white gaussian noise, 0dB variance
    #Noise addition
    ...
    cipSoftM = np.reshape(np.real(y),(int(N/4),7))
